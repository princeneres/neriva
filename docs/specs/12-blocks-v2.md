...
Pure functions, server-safe (no DOM): tokenize the html string; substitute `{{prop}}` escaped; locate binding elements (leaf constraint makes regex-based inner-content replacement safe); build the slot tree; render it. Scope css by prefixing each top-level selector with `[data-nv-b="<erc>"]` and emit one `<style>` per distinct block ERC on the page. Used by: public route, studio canvas, block preview.

**Slot tree (amendment, replaces flat segments)**: `renderTemplate` returns `nodes: TemplateNode[]`, a tree of `{ kind: 'html' }`, `{ kind: 'element', tag, attrs, children }` and `{ kind: 'slot', name, tag, attrs }`, built by `buildTemplateTree`. It previously returned a flat list of sibling segments cut at each slot, which **could not express nesting**: an element whose closing tag fell into a later segment was auto-closed by the HTML parser as soon as that segment was injected, so every layout block lost its wrapper. A page built on `nv-container` rendered an empty section followed by unwrapped content, with `.nv-container-inner` (the centered, max-width column) gone entirely, and `nv-columns-2`/`nv-columns-3` never wrapped their columns so the grid did not apply.

Rules of the tree: only elements that contain a slot are materialised as `element` nodes, so markup with no slot inside stays one raw html run and slotless blocks keep exactly the previous single-injection path; those runs are therefore always balanced, which is what makes injecting them safe. Ancestor matching counts depth rather than taking the next closing tag, because a slot's ancestors can nest the same tag (`nv-columns-2` is a div of divs) while the leaf-only binding constraint that justifies the shortcut elsewhere does not apply to them. A slot node **keeps its own tag and attributes**, including `data-nv-slot` (the studio locates slot containers by it) and its classes, which is what restores `.nv-container-inner` and `.nv-columns-2-col`. Undeclared slot names are dropped at any depth.

`renderTemplateNodes` (`lib/renderer/template-nodes.tsx`) turns the tree into React elements, mapping `class` to `className`, `for` to `htmlFor` and a `style` string to the object React requires (splitting on semicolons only, so a `var(--token, fallback)` survives). It takes an optional renderer for the raw html runs, which the studio canvas uses to swap in its contentEditable wrapper while sharing the traversal, so the canvas and the published page cannot drift apart on nesting.
